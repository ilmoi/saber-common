...
const TransactionEnvelope_1 = require("./transaction/TransactionEnvelope");
...
class SolanaProvider extends SolanaReadonlyProvider {
    ...
    static init({ connection, broadcastConnections = [connection], wallet, opts = exports.DEFAULT_PROVIDER_OPTIONS, }) {
        ...
        return new SolanaProvider(connection, broadcastConnections.length > 1
            ? new index_1.TieredBroadcaster(connection, broadcastConnections, opts)
            : new broadcaster_1.SingleConnectionBroadcaster(firstBroadcastConnection, opts), wallet, opts);
    }
    /**
     * Sends the given transaction, paid for and signed by the provider's wallet.
     *
...
class SolanaAugmentedProvider {
    ...
    newTX(instructions = [], signers = []) {
        return TransactionEnvelope_1.TransactionEnvelope.create(this, instructions, signers);
    }
    /**
    ...
    async requestAirdrop(lamports, to = this.wallet.publicKey) {
        return new index_1.PendingTransaction(this.connection, await this.connection.requestAirdrop(to, lamports));
    }
    /**
     ...
     */
    withSigner(signer) {
        return new SolanaAugmentedProvider(new SolanaProvider(this.connection, this.broadcaster, new index_1.SignerWallet(signer), this.opts));
    }
}
